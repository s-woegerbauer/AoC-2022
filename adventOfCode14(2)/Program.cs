...
for(int i = 0; i < coordinatesAlreadyThere.Count; i++)
{
    max = Math.Max(max, int.Parse(coordinatesAlreadyThere[i].Split(',')[1]));
}
...
{
    coordinatesAlreadyThere.Add($"{i},{max}");
}

while(!coordinatesAlreadyThere.Contains("500,0"))
{
